No external software installation required.

First move the tar file to the Desktop.

Then extract the tar folder to with the command "tar -xvf cuACS.tar".

Now navigate to the newly extracted folder in terminal.

Use the build script to make the project.

Run the command "./buildScript.txt" to build the project.

Use run command "./cuACS" to run the program.

To view the detailed information of a client or animal, click on it's name when viewing the
list of either animals or staff.
